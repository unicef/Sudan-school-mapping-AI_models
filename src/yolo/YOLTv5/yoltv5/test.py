...
results_dir = os.path.join(config.project_folder, config.outproject_infer, config.outname_infer)
...
# slice test images
###################
outdir_slice_ims = os.path.join(config.project_folder, config.outdir_slice_ims)
outpath_test_txt = os.path.join(config.project_folder, config.outdir_slice_txt, config.outpath_test_txt)
...
if config.sliceWidth > 0:
    ...
    if not os.path.exists(outdir_slice_ims):  
        ...
        os.makedirs(os.path.join(config.project_folder, config.outdir_slice_txt)) #, exist_ok=True) 
        ...
        im_list_test = []
        for f in sorted([z for z in os.listdir(outdir_slice_ims) if z.endswith(config.out_ext)]):
        ...
        df_tmp = pd.read_csv(outpath_test_txt, names=['path']) 
...
yolov5_infer_output_folder = os.path.join(config.project_folder, config.outproject_infer)

yolt_cmd = 'python {} --weights {} --source {} --img {} --conf {} ' \
            '--project {} --name {} --nosave --save-txt --save-conf'.format(\
            script_path, config.weights_file, outdir_slice_ims,
            config.train_im_size, min(config.detection_threshes), 
            yolov5_infer_output_folder, config.outname_infer)
...
out_dir_root = os.path.join(config.project_folder, config.results_folder)
os.makedirs(out_dir_root, exist_ok=True)
print("post-proccessing:", config.outname_infer)

# if geojson files dir is an absolute path
if config.src_geojson_dir.startswith('/'):
    src_geojson_dir = config.src_geojson_dir

else: # if it is relative to prediction output folder
    src_geojson_dir = os.path.join(config.project_folder, config.src_geojson_dir)
...
for detection_thresh in config.detection_threshes:
    ...
    post_process.execute(
        ...
        orig_geojson_file = config.orig_geojson_file,  
