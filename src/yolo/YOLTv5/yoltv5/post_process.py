...
def extract_geojsons(geojson_dir, output_folder, file_id_list):
    """
    Produce geojson file for each image tile in which target objects were detected by the model.
    ...
    Params:
        ...
        file_id_list (list): ids of images in which target objects were detected
    ...
    for file_id in tqdm(file_id_list):
        ...
        row = data[data['id'] == int(file_id)]
        save_dir = os.path.join(output_folder, file_id + '.geojson')
        row.to_file(save_dir, driver='GeoJSON')
...
def execute(pred_dir='/root/yoltv5/results/',
            truth_file='',
            raw_im_dir='/wdata',
            src_geojson_dir = None,
            orig_geojson_file = None,  
            im_ext='.tif',
            ignore_names=[],
    ...
    # generate geojson files according to images files in which the model detected bounding boxes
    # extract_geojsons(geojson_dir = orig_geojson_file, 
    #                  output_folder = src_geojson_dir, 
    #                  file_id_list = list(im_names_set))
    ...
    for i,im_name in enumerate(im_names_tiled):
        ...
        # get all boxes for this image, create a list of box names too
        # if im_name in im_names_set:
        else:
            # get image height/width for geo-referencing
            img = cv2.imread(im_path)
            image_height, image_width = img.shape[:2]

            # read a geojson file for each image tile
            geojson_path = os.path.join(src_geojson_dir, im_name + '.geojson' )
            image_id, crs, left, top, right, bottom = parse_geojson(geojson_path)
            affine_obj = rio.transform.from_bounds(west = left, south = bottom, east = right, north = top,
            ...
            outfile_geojson_geo_orig_crs_centroid = os.path.join(geojson_geo_dir, im_name + '_' + crs_str + '_centroid.geojson')
            ...
            geom_list_geo_centroid, geom_list_pix_centroid = [], []
            box_names = []
            for j, bbox in enumerate(boxes):
                ...
                geom_geo = convert_poly_coords(geom_pix, raster_src=None, 
                                        affine_obj=affine_obj, inverse=False)
                
                
                geom_list_geo.append([geom_geo, classs, prob])
                geom_list_pix.append([geom_pix, classs, prob])
                
                geom_list_geo_centroid.append([geom_geo.centroid, classs, prob]) 
                geom_list_pix_centroid.append([geom_geo, classs, prob])
            ...
            gdf_geo_centroid = gpd.GeoDataFrame(geom_list_geo_centroid, columns=['geometry', 'category', 'prob'], crs=crs)
            # add pix coords to geo gdf as well
            gdf_geo_centroid['x0_pix'] = df_filt['Xmin_Glob'].values
            gdf_geo_centroid['x1_pix'] = df_filt['Xmax_Glob'].values
            gdf_geo_centroid['y0_pix'] = df_filt['Ymin_Glob'].values
            gdf_geo_centroid['y1_pix'] = df_filt['Ymax_Glob'].values
            gdf_geo_centroid.to_file(outfile_geojson_geo_orig_crs_centroid, driver='GeoJSON')
...
if __name__ == "__main__":
    ...
    src_geojson_dir = os.path.join(args.project_folder, args.src_geojson_dir)
    
    execute(pred_dir=args.pred_dir,
            truth_file=args.truth_file,
            # sliced_im_dir=args.sliced_im_dir,
            raw_im_dir=args.raw_im_dir,
            src_geojson_dir = src_geojson_dir,
            orig_geojson_file = args.orig_geojson_file,
